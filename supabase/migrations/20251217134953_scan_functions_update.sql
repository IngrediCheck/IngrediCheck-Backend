drop function if exists "public"."get_scans"(p_limit integer, p_offset integer);
...
CREATE OR REPLACE FUNCTION public.get_scan_detail(p_scan_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.get_scans(p_limit integer DEFAULT 20, p_offset integer DEFAULT 0, p_favorited boolean DEFAULT NULL::boolean)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.is_analysis_stale(p_food_note_snapshot jsonb)
 RETURNS boolean
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.submit_feedback(p_target_type text, p_vote_type text DEFAULT 'down'::text, p_scan_id uuid DEFAULT NULL::uuid, p_scan_image_id uuid DEFAULT NULL::uuid, p_scan_analysis_id uuid DEFAULT NULL::uuid, p_ingredient_name text DEFAULT NULL::text, p_comment text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.toggle_scan_favorite(p_scan_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
