...
AS $function$
    select case
        ...
        when input_barcode <> regexp_replace(input_barcode, '[^0-9]', '', 'g')
            then normalized_barcode(regexp_replace(input_barcode, '[^0-9]', '', 'g'))
...
alter table "public"."review_expected_outputs" add constraint "review_expected_outputs_thread_id_fkey" FOREIGN KEY (thread_id) REFERENCES public.review_threads(id) ON DELETE CASCADE not valid;
...
CREATE OR REPLACE FUNCTION public.barcode_review_list(p_limit integer DEFAULT 50, p_status text DEFAULT NULL::text, p_offset integer DEFAULT 0, p_search_query text DEFAULT NULL::text)
 RETURNS TABLE(thread_id text, subject_id text, product_name text, barcode text, category text, output_interpretation jsonb, status text, latency_ms numeric, created_at timestamp with time zone, reviewer_ids uuid[], comment_count bigint, expected_output jsonb)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  SELECT 
    bd.thread_uuid::text AS thread_id,
    COALESCE(bd.client_activity_id::text, bd.activity_id::text) AS subject_id,
    COALESCE(
      bd.inventory_name, 
      CASE WHEN bd.response_status = 200 THEN bd.response_body->>'name' ELSE NULL END,
...
CREATE OR REPLACE FUNCTION public.extract_review_list_enhanced(p_limit integer DEFAULT 50, p_status text DEFAULT NULL::text, p_offset integer DEFAULT 0, p_search_query text DEFAULT NULL::text)
 RETURNS TABLE(thread_id text, subject_id text, product_name text, barcode text, category text, output_json text, input_json text, status text, latency_ms numeric, created_at timestamp with time zone, reviewer_ids uuid[], reviewers jsonb, comment_count bigint, expected_output jsonb)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  SELECT 
    ed.thread_uuid::text AS thread_id,
    COALESCE(ed.client_activity_id::text, ed.activity_id::text) AS subject_id,
    COALESCE(ed.name, 'Unknown Product')::text AS product_name,
    COALESCE(ed.barcode, 'N/A')::text AS barcode,
    'Extract'::text AS category,
...
CREATE OR REPLACE FUNCTION public.preferences_review_list(p_limit integer DEFAULT 50, p_status text DEFAULT NULL::text, p_offset integer DEFAULT 0, p_search_query text DEFAULT NULL::text)
 RETURNS TABLE(thread_id text, subject_id text, input_json text, output_json text, status text, latency_ms numeric, created_at timestamp with time zone, reviewer_ids uuid[], comment_count bigint, expected_output jsonb)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE TRIGGER trg_inventory_cache_updated_at BEFORE UPDATE ON public.inventory_cache FOR EACH ROW EXECUTE FUNCTION public.set_inventory_cache_updated_at();

CREATE TRIGGER trg_review_expected_outputs_set_updated_at BEFORE UPDATE ON public.review_expected_outputs FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
with check ((bucket_id = 'productimages'::text));
  ...
  for delete
  to authenticated
using ((bucket_id = 'memoji-images'::text));
...
using ((bucket_id = 'memoji-images'::text));
  ...
  for insert
  to authenticated
with check ((bucket_id = 'memoji-images'::text));
