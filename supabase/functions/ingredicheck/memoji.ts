...
// Get or initialize memoji generation count for a user.
// Updated on each successful memoji generation via recordMemojiGeneration.
async function getOrInitMemojiUsage(
    client: SupabaseClient,
    userId: string,
): Promise<{ memoji_generation_count: number }> {
    const { data, error } = await client
        .from("users")
        .select("memoji_generation_count")
        .eq("user_id", userId)
        .single();
...
// Record a memoji generation for the user (increments count).
async function recordMemojiGeneration(client: SupabaseClient, userId: string): Promise<void> {
    const usage = await getOrInitMemojiUsage(client, userId);
    const { error } = await client
        .from("users")
        .update({
            memoji_generation_count: usage.memoji_generation_count + 1,
        })
        .eq("user_id", userId);
...
export function registerMemojiRoutes(router: Router, serviceClient: SupabaseClient | null) {
    router.post("/ingredicheck/memoji", async (ctx: MemojiContext) => {
        ...
        // Updated on each successful memoji generation.
        try {
            await recordMemojiGeneration(supabase, userId);
        } catch (e) {
